...
`section-01/front/src/index.html`を編集すれば、自動的にブラウザの表示が更新されます。

## まずは書いてみましょう！
`section-01/front/src/index.html`の内容は、以下のようになっていると思います。ここに何が書いてあるかは今後説明していくので一旦置いておいて、まずは実際にHTMLを書いてみましょう！書けば分かるようになります！
...
また、タグは入れ子にする (タグの中にタグを入れる) ことができます。divタグの中にpタグが入っている状態は、以下のように表されます。
```html
<div>
  <p></p>
</div>
...
どのタグの中にどのタグが入るのか、もちろん全て決まっていますが、そのあたりは今度扱います。
